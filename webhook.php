<?php
// webhook.php - Recibe notificaciones de Mercado Pago y guarda pagos en MySQL
...
if (isset($data['type']) && $data['type'] === 'payment' && isset($data['data']['id'])) {
    ...
    // === Si el pago está aprobado, lo insertamos ===
    if (isset($payment['status']) && $payment['status'] === 'approved') {
        ...
        if ($check->rowCount() === 0) {
            ...
            file_put_contents('mp_log.txt', date('Y-m-d H:i:s') . " - Pago insertado: $id_pago\n", FILE_APPEND);
        } else {
            file_put_contents('mp_log.txt', date('Y-m-d H:i:s') . " - Pago ya existe: $id_pago\n", FILE_APPEND);
        }
    } else {
